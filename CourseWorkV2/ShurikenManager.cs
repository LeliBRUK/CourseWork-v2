    ...
    internal class ShurikenManager
    {
        ...
        private static void AddShuriken(ThiefRed p)
        {
            ...
            ShurikenAnimation.Initialize(ShurikenTexture, p.Position,
                ShurikenTexture.Width,
                ShurikenTexture.Height,
                1,
                30,
                Color.White,
                1f,
        ...
        public void UpdateShurikenManager(GameTime gameTime, ThiefRed p, KnightBlue k)
        {
            ...
            {
                Thief.shurikenSound.SOUND_INSTANCE.Play();
            ...
            foreach(Shurikens S in ShurikenManager.Shuriken)
            {
                ShurikenRectangle=new Rectangle(
                    (int)S.Position.X,
                    (int)S.Position.Y,
                    S.Width, S.Height );
                ...
                if (ShurikenRectangle.Intersects(EnemyRectangle))
                {
                    if(k.Health > 0)
                    {
                        k.Health--;
                        if (k.Health > 0)
                          Knight.hurtSound.SOUND_INSTANCE.Play();
                        else
                          Knight.deathSound.SOUND_INSTANCE.Play();
                    }
                    S.Active = false;
                }
            }
        ...
        public void UpdateShurikenManager2(GameTime gameTime, ThiefBlue p,KnightRed k)
        {
            previousKeyboardState = currentKeyboardState;
            currentKeyboardState = Keyboard.GetState();
